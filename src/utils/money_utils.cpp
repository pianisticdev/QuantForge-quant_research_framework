...
namespace money_utils {
    Money Money::parse_microdollars(const std::string& money_string) {
        ...
        if (decimal_position == std::string_view::npos) {
            return Money(std::stoll(money_string) * scaled_base);
        }
        ...
        return Money(std::stoll(whole_part) * scaled_base + std::stoll(fractional_part));
    };
}  // namespace money_utils
